  ...
  const scan = useCallback(
    async (
      ...
      try {
        ...
        if (HAS_CHROME) {
          setLoadingMessage(
            "Looking for the actual privacy policy page..."
          );

          link =
            await findPolicyLink(
              t.id
            );

          console.log(
            "PrivacyLens: policy link found ->",
            link
          );
        ...
        if (
          ...
          setError({
            ...
            message:
              "Try opening the site's actual privacy policy page.",
          });
        } else if (
          msg ===
          "NOT_A_POLICY"
          ...
          setError({
            title:
              "Couldn't find a privacy policy on this site",
            message:
          ...
          setError({
            title:
              "Can't reach the PrivacyLens server",
            message:
              "The PrivacyLens production server could not be reached. Please try again.",
          });
        } else if (
          msg === "TIMEOUT"
        ) {
          ...
          setChatLog(
            (log) => [
              ...log,
              {
                role: "ai",
                text:
                  "Couldn't reach the PrivacyLens server.",
              },
            ]
          );
        } finally {
